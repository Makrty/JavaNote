...
// 所有人都知道， 如果 n 是偶数， 这个表达式为 0 ; 如果 n 是奇数， 表达式则为 1。 
// 当然， 除 非 n 是负数 如果 n 为负， 这个表达式则为 -1。 为什么呢? 
//  设计最早的计算机时， 必须有人 制定规则， 明确整数除法和求余对负数操作数该如何处理。
//  数学家们几百年来都知道这样一 个最优(或“ 欧几里德”)规则:余数总是要& 0。不过，最早制定规则的人并没有翻开数学 书好好研究， 
//  而是提出了一些看似合理但实际上很不方便的规则。 
// 下面考虑这样一个问题: 计算一个时钟时针的位置。
// 这里要做一个时间调整， 而且要归 一化为一个 0 ~ 11 之间的数。 
// 这很简单:(position + adjustment) % 12。 不过， 如果这个调 整为负会怎么样呢? 
//  你可能会得到一个负数。 所以要引入一个分支， 或者使用((position + adjustment) % 12 + 12) % 12。不管怎样， 总之都很麻烦。 
// floorMod 方法就让这个问题变得容易了: floorMod(position + adjustment, 12) 总会得到一 个 0 ~ 11 之间的数。
// (遗憾的是， 对于负除数， floorMod 会得到负数结果， 不过这种情况在 实际中很少出现。) 
 
